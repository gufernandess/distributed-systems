...
public class Server {
    ...
    private static class ClienteHandler implements Runnable {
        ...
        private void tratarEleitor(InputStream entrada, OutputStream saida)
            throws IOException {
            ListaCandidatos.Builder lista = ListaCandidatos.newBuilder();
            for (CandidatoData c : candidatos.values()) {
                lista.addCandidatos(
                    Candidato.newBuilder().setId(c.id).setNome(c.nome)
                );
            }
            ...
            if (
                voto != null &&
                candidatos.containsKey(voto.getCandidatoId()) &&
                votacaoAberta
            ) {
        ...
        private void tratarAdmin(InputStream entrada, OutputStream saida)
            throws IOException {
            while (true) {
                ...
                if (cmd.hasAdd()) {
                    ...
                    candidatos.put(
                        id,
                        new CandidatoData(id, cmd.getAdd().getNome())
                    );
                    System.out.println(
                        "Admin adicionou candidato: " + cmd.getAdd().getNome()
                    );
                } else if (cmd.hasRemove()) {
                    candidatos.remove(cmd.getRemove().getId());
                    System.out.println(
                        "Admin removeu candidato: " + cmd.getRemove().getId()
                    );
                } else if (cmd.hasNota()) {
